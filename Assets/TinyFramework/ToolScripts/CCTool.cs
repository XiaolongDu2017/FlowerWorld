...
public static class CCTool
{
    ...
    {
        return new Color(Mathf.Round(pColor.r * 255f), Mathf.Round(pColor.g * 255f), Mathf.Round(pColor.b * 255f),
            Mathf.Round(pColor.a * 255f));
    }
    ...
    public static Sprite ResourcesLoadSprite(string pPath)
    {
        var tSprite = Resources.Load<Sprite>(pPath);
        return tSprite;
    }
    ...
    {
        return string.Format("{0}年{1}月{2}日{3}时{4}分{5}秒", pTime.Year, pTime.Month, pTime.Day, pTime.Hour, pTime.Minute,
            pTime.Second);
    }
    ...
    public static Rect AutoAdaptImage(RectTransform pRect, Texture2D pTargetImage)
    {
        ...
        bool pBaseWeightOrHeight = (((float) pTargetImage.height / (float) pTargetImage.width) >
                                    (pRect.GetHeight() / pRect.GetWidth()));
